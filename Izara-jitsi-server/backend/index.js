...
import { createPostMeetingPipeline } from './services/postMeetingPipeline.js';
...
// CORS - CORS_ORIGINS env (+ localhost/LAN defaults). Supports glob patterns with `*`.
const izaraCorsPolicyForExport = izaraCorsPolicy;
const ALLOWED_ORIGINS = [...izaraCorsPolicyForExport.literals];

function isOriginAllowed(origin) {
  return isIzaraOriginAllowed(origin, izaraCorsPolicyForExport);
}
...
function getHostPresence(meetingId) {
  const keys = meetingSocketRoomIds(meetingId);
  for (const key of keys) {
    const entry = meetingHostsOnline.get(key);
...
function markHostOnline(meetingId, { inJitsi = false } = {}) {
  if (!meetingId) return;
  if (!inJitsi) {
    console.warn('[Meeting] host-present ignored — requires inJitsi:true after doctor videoConferenceJoined');
    return;
  }
  const keys = meetingSocketRoomIds(meetingId);
  const payload = { meetingId, ready: true, inJitsi: true, at: new Date().toISOString() };
  for (const key of keys) {
...
function markHostAbsent(meetingId) {
  if (!meetingId) return;
  const keys = meetingSocketRoomIds(meetingId);
  const payload = { meetingId, ready: false, inJitsi: false, at: new Date().toISOString() };
  for (const key of keys) {
    meetingHostsOnline.delete(key);
    io.to(key).emit('host-absent', payload);
  }
}

function replayHostReadyIfPresent(meetingId) {
  if (!isHostReadyForMeeting(meetingId)) return;
  const presence = getHostPresence(meetingId);
  if (!presence) return;
  const keys = meetingSocketRoomIds(meetingId);
  for (const key of keys) {
    io.to(key).emit('host-ready', { ...presence, meetingId });
  }
}
...
/** Resolve display name / id from Izara session + meeting record (guest uses invite/manual name only). */
async function resolveMeetingParticipant(req, meetingId, role) { // NOSONAR S3776 — multi-source identity resolution
  const roleNorm = String(role || 'guest').toLowerCase();
  const isHostRole = roleNorm === 'doctor' || roleNorm === 'admin' || roleNorm === 'host';
  const isPatientRole = roleNorm === 'patient';
  const isGuestRole = roleNorm === 'guest';
  ...
  if (isGuestRole) {
    ...
    participantId = req.user.id || req.user.sub || req.user.userId || '';
  }

  if (dbAvailable && !isGuestRole) {
    try {
      const result = await pool.query(
      ...
      if (row) {
        if (isHostRole) {
          displayName = row.doctor_display || displayName || 'Doctor';
          if (!email) email = row.doctor_email || '';
          if (!participantId) participantId = row.doctor_id || '';
        } else if (isPatientRole) {
          displayName = row.patient_display || displayName || 'Patient';
...
app.post('/api/meetings/:id/lobby/join', optionalAuth, async (req, res) => { // NOSONAR S3776 — lobby join with role validation, duplicate detection, socket notify
  ...
  let participantId = req.body.participantId || `guest-${uuidv4().substring(0, 8)}`;
  ...
  // Authenticated patients always lobby under their session user id (ignore spoofed body ids)
  if (req.user && authenticatedRole === 'patient') {
    participantId = req.user.id || req.user.userId || req.user.patientId || participantId;
  }
...
app.post('/api/meetings/:id/host-present', optionalAuth, (req, res) => {
  const inJitsi = req.body?.inJitsi === true;
  markHostOnline(req.params.id, { inJitsi });
  const ready = isHostReadyForMeeting(req.params.id);
  res.json({ success: true, ready, inJitsi: ready });
});

app.post('/api/meetings/:id/host-absent', optionalAuth, (req, res) => {
  markHostAbsent(req.params.id);
  res.json({ success: true, ready: false, inJitsi: false });
});
...
// E2E/dev: clear in-memory lobby + host-ready for isolated lobby edge-case tests
if (IS_DEV_TESTING) {
  app.post('/api/meetings/:id/dev/reset-lobby', optionalAuth, async (req, res) => {
    const { id } = req.params;
    const lobbyKey = await resolveLobbyKey(id);
    resetMeetingSessionState(id, lobbyKey);
    res.json({ success: true, meetingId: id, lobbyKey });
  });
}
