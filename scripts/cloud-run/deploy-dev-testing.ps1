#!/usr/bin/env pwsh
# ============================================================================
# IZARA TELEMEDICINE — Cloud Run Dev-Testing Deployment Script
# Version: 1.4.8
# ============================================================================
#
# Deploys all 5 services to Google Cloud Run with "-dev-testing" suffix:
#   1. izara-postgres-dev-testing     (PostgreSQL 18 + pgvector)
...
#
# Architecture: Same as local Docker — PostgreSQL on Cloud Run (not Cloud SQL)
#
# Usage:
#   .\deploy-dev-testing.ps1                    # Deploy all services
#   .\deploy-dev-testing.ps1 -SkipBuild         # Deploy without rebuilding images
#   .\deploy-dev-testing.ps1 -OnlyPostgres      # Deploy only PostgreSQL
#   .\deploy-dev-testing.ps1 -Teardown          # Delete all dev-testing services
#
# ============================================================================
...
# Image names (PostgreSQL runs on GCE VM, pgAdmin uses public image directly)
...
# Database credentials (same as local Docker)
$DB_USER        = "postgres"
...
if ($Teardown) {
    ...
    Write-Step "Deleting PostgreSQL VM (izara-postgres-dev-testing)..."
    gcloud compute instances delete izara-postgres-dev-testing --zone=asia-southeast1-b --quiet 2>$null
    if ($LASTEXITCODE -eq 0) { Write-OK "Deleted VM" }
    else { Write-Warn "VM not found or already deleted" }
...
Write-Host "  IZARA TELEMEDICINE v1.4.8 — Cloud Run Dev-Testing Deployment" -ForegroundColor Magenta
...
# ============================================================================
# PostgreSQL runs on a GCE VM (not Cloud Run) because Cloud Run's HTTP proxy
# doesn't support PostgreSQL's binary wire protocol.
# VM: izara-postgres-dev-testing (e2-small, asia-southeast1-b)
# IP: 35.240.162.227 (update this if VM is recreated)
# ============================================================================
...
if ($OnlyPostgres) {
    ...
    if ($vmStatus -eq "RUNNING") {
        Write-OK "PostgreSQL VM is RUNNING at $PG_HOST"
    } else {
...
# ============================================================================
# STEP 2: DEPLOY PGADMIN TO CLOUD RUN
# ============================================================================
if (-not $OnlyApps) {
    ...
    $PGADMIN_URL = Get-ServiceUrl $SVC_PGADMIN
    Write-OK "pgAdmin deployed: $PGADMIN_URL"
    Write-OK "Add server manually: Host=$PG_HOST, Port=5432, User=postgres, Password=IzaraDb2024"
}

if ($OnlyPgAdmin) {
    Write-Host "`n✅ pgAdmin deployment complete!" -ForegroundColor Green
    Write-Host "  URL: $(Get-ServiceUrl $SVC_PGADMIN)"
    exit 0
}
...
# ============================================================================
# STEP 4: BUILD & DEPLOY MEETING SERVER
# ============================================================================
if (-not $SkipBuild) {
    Write-Step "Building Meeting Server image..."
    Push-Location "$ROOT_DIR\Izara-jitsi-server"
    
    docker build -t $IMG_MEETING .
    if ($LASTEXITCODE -ne 0) { Write-Err "Meeting Server build failed!"; Pop-Location; exit 1 }
    
    docker push $IMG_MEETING
    if ($LASTEXITCODE -ne 0) { Write-Err "Meeting Server push failed!"; Pop-Location; exit 1 }
    Write-OK "Meeting Server image pushed: $IMG_MEETING"
    Pop-Location
}
...
# ============================================================================
# STEP 5: BUILD & DEPLOY PATIENT PORTAL
# ============================================================================
if (-not $SkipBuild) {
    Write-Step "Building Patient Portal image..."
    Push-Location "$ROOT_DIR\Isara-patient-portal"
    
    # Get the meeting server URL for build args
    docker build -f Dockerfile.unified `
        --build-arg VITE_API_URL="" `
        --build-arg VITE_APP_NAME="Izara Patient Portal (Dev)" `
        --build-arg VITE_APP_VERSION="1.4.8-dev" `
        --build-arg VITE_APP_ENV=production `
        --build-arg VITE_USE_POSTGRESQL=true `
        --build-arg VITE_MEETING_SERVER_URL=$MEETING_URL `
    ...
    if ($LASTEXITCODE -ne 0) { Write-Err "Patient Portal push failed!"; Pop-Location; exit 1 }
    Write-OK "Patient Portal image pushed: $IMG_PATIENT"
    Pop-Location
}
...
# ============================================================================
# STEP 6: BUILD & DEPLOY DOCTOR PORTAL
# ============================================================================
if (-not $SkipBuild) {
    Write-Step "Building Doctor Portal image..."
    Push-Location "$ROOT_DIR\Isara-doctor-portal"
    
    docker build -f Dockerfile.unified `
        --build-arg VITE_API_URL="" `
        --build-arg VITE_AUTH_URL="" `
        ...
        --build-arg VITE_MEETING_SERVER_URL=$MEETING_URL `
        -t $IMG_DOCTOR .
    if ($LASTEXITCODE -ne 0) { Write-Err "Doctor Portal build failed!"; Pop-Location; exit 1 }
    
    docker push $IMG_DOCTOR
    if ($LASTEXITCODE -ne 0) { Write-Err "Doctor Portal push failed!"; Pop-Location; exit 1 }
    Write-OK "Doctor Portal image pushed: $IMG_DOCTOR"
    Pop-Location
}
...
# ============================================================================
# STEP 7: UPDATE CORS FOR MEETING SERVER
# ============================================================================
Write-Step "Updating Meeting Server CORS with portal URLs..."
gcloud run services update $SVC_MEETING `
    --region=$REGION `
    --update-env-vars="CORS_ORIGINS=$PATIENT_URL,$DOCTOR_URL" `
    --quiet

Write-OK "Meeting Server CORS updated"
...
Write-Host "  DEPLOYMENT COMPLETE — Izara Telemedicine v1.4.8 Dev-Testing" -ForegroundColor Green
Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Green
...
Write-Host "  🔧 pgAdmin:         $PGADMIN_URL" -ForegroundColor White
Write-Host "  🏥 Patient Portal:  $PATIENT_URL" -ForegroundColor White
Write-Host "  👨‍⚕ Doctor Portal:   $DOCTOR_URL" -ForegroundColor White
Write-Host "  🎥 Meeting Server:  $MEETING_URL" -ForegroundColor White
Write-Host ""
...
Write-Host "     .\deploy-dev-testing.ps1 -Teardown"
Write-Host ""
Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Green
