...
-- AI Chat Memory (Long-term summarized memory with vector embeddings for RAG)
...
-- Transcript Embeddings (Vectorized meeting transcript chunks for doctor AI search)
...
CREATE INDEX idx_ai_chat_embedding ON ai_chat_history USING ivfflat (embedding vector_cosine_ops);
CREATE INDEX idx_cds_logs_patient_id ON cds_logs(patient_id);
CREATE INDEX idx_cds_logs_doctor_id ON cds_logs(doctor_id);

-- AI Chat Memory indexes (vector + lookup)
CREATE INDEX idx_ai_memory_user_id ON ai_chat_memory(user_id);
CREATE INDEX idx_ai_memory_type ON ai_chat_memory(memory_type);
CREATE INDEX idx_ai_memory_embedding ON ai_chat_memory USING ivfflat (embedding vector_cosine_ops);
CREATE INDEX idx_ai_memory_active ON ai_chat_memory(is_active) WHERE is_active = true;

-- Transcript Embedding indexes (vector search for doctor AI)
CREATE INDEX idx_transcript_emb_meeting ON transcript_embeddings(meeting_record_id);
CREATE INDEX idx_transcript_emb_patient ON transcript_embeddings(patient_id);
CREATE INDEX idx_transcript_emb_doctor ON transcript_embeddings(doctor_id);
CREATE INDEX idx_transcript_emb_vector ON transcript_embeddings USING ivfflat (embedding vector_cosine_ops);
...
-- =============================================================================
-- SECTION 26: PHASE 2 - DEVICE TOKENS (Push Notification Registration)
-- =============================================================================
\echo '>>> Creating Phase 2 tables...'
...
-- =============================================================================
-- SECTION 27: PHASE 2 - BIOMETRIC CREDENTIALS
-- =============================================================================
...
-- =============================================================================
-- SECTION 28: PHASE 2 - REFRESH TOKENS
-- =============================================================================
...
-- =============================================================================
-- SECTION 29: PHASE 2 - PUSH SUBSCRIPTIONS & NOTIFICATION PREFERENCES
-- =============================================================================
...
-- =============================================================================
-- SECTION 30: PHASE 2 - MULTI-API CONNECTIONS
-- =============================================================================
...
-- =============================================================================
-- SECTION 31: PHASE 2 - OFFLINE SYNC QUEUE
-- =============================================================================
...
-- =============================================================================
-- SECTION 32: PHASE 2 - USER SETTINGS
-- =============================================================================
...
('DT-DEMO-01', 'PATIENT-DEMO', 'expo-push-token-demo-web', 'web', 'E2E Test Browser', 'Chrome 130', 'Windows 11', '2.0.0')
...
('NP-DM-PUSH-APT', 'PATIENT-DEMO', 'push', 'appointment', true),
('NP-DM-PUSH-MED', 'PATIENT-DEMO', 'push', 'medication', true),
('NP-DM-EMAIL-APT', 'PATIENT-DEMO', 'email', 'appointment', true),
('NP-DM-EMAIL-LAB', 'PATIENT-DEMO', 'email', 'lab_result', true)
ON CONFLICT DO NOTHING;
...
UNION ALL SELECT 'Device Tokens', count(*) FROM device_tokens
UNION ALL SELECT 'Push Subscriptions', count(*) FROM push_subscriptions
UNION ALL SELECT 'API Connections', count(*) FROM user_api_connections
UNION ALL SELECT 'User Settings', count(*) FROM user_settings
UNION ALL SELECT 'Notification Prefs', count(*) FROM notification_preferences;
...
\echo 'Phase 1 + Phase 2 tables initialized.'
\echo '=============================================='
