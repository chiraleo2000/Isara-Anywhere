/**
 * Clinical Resources & Medical Library Workflows Test - API Based
...
test.describe('1. Authentication for Clinical Resources', () => {
  test('1.1 Doctor login - 200', async ({ request }) => {
    ...
    console.log('✅ Doctor authenticated for clinical resources');
  });
  ...
  test('1.2 Admin login - 200', async ({ request }) => {
    ...
    console.log('✅ Admin authenticated for clinical resources');
  });
...
test.describe('2. Clinical Resources API', () => {
  ...
  test('2.1 Doctor can access clinical resources - 200', async ({ request }) => {
  ...
  test('2.4 Doctor can access patients - 200', async ({ request }) => {
...
test.describe('3. Admin Clinical Resources API', () => {
  test.beforeAll(async ({ request }) => {
    if (!adminToken) {
  ...
  test('3.1 Admin can access clinical resources - 200', async ({ request }) => {
    const response = await request.get(`${DOCTOR_PORTAL_URL}/api/clinical-resources`, {
  ...
  test('3.2 Admin can access medical content - 200', async ({ request }) => {
    const response = await request.get(`${DOCTOR_PORTAL_URL}/api/medical-content`, {
      headers: { Authorization: `Bearer ${adminToken}` }
    });
    expect(response.status()).toBe(200);
  ...
  test('3.3 Admin can access appointments - 200', async ({ request }) => {
  ...
  test('3.4 Admin can access notifications - 200', async ({ request }) => {
...
test.describe('4. Patient Medical Library API', () => {
  test.beforeAll(async ({ request }) => {
    if (!patientToken) {
