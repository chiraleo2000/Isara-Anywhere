...
test.describe('04 — Video Meeting, Transcription & AI Summary', () => {
  ...
  test.describe('B — Meeting Join & Lobby', () => {
    ...
    test('B09 — Patient and Doctor open meeting pages simultaneously', async ({ browser }) => {
      ...
      await Promise.all([
        patientPage.goto(`${PATIENT_URL}/appointments`, { timeout: TIMEOUTS.navigation }),
        doctorPage.goto(`${DOCTOR_URL}/appointments`, { timeout: TIMEOUTS.navigation }),
      ]);
      await Promise.all([patientPage.waitForTimeout(2000), doctorPage.waitForTimeout(2000)]);

      await Promise.all([
        screenshot(patientPage, '23-B09-patient-meeting'),
        screenshot(doctorPage, '23-B09-doctor-meeting'),
      ]);
  ...
  test.describe('F — Multi-Browser Meeting Simulation', () => {
    ...
    test('F02 — 3-way meeting simulation (doctor + patient + admin)', async ({ browser }) => {
      ...
      await Promise.all([
        loginViaBrowser(pages[0], 'doctor'),
        loginViaBrowser(pages[1], 'patient1'),
        loginViaBrowser(pages[2], 'admin'),
      ]);

      await Promise.all([
        pages[0].goto(`${DOCTOR_URL}/appointments`, { timeout: TIMEOUTS.navigation }),
        pages[1].goto(`${PATIENT_URL}/appointments`, { timeout: TIMEOUTS.navigation }),
        pages[2].goto(`${DOCTOR_URL}/appointments`, { timeout: TIMEOUTS.navigation }),
      ]);

      await Promise.all(pages.map(p => p.waitForTimeout(2000)));
      await Promise.all([
        screenshot(pages[0], '23-F02-doctor-3way'),
        screenshot(pages[1], '23-F02-patient-3way'),
        screenshot(pages[2], '23-F02-admin-3way'),
      ]);
