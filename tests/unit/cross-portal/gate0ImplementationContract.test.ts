...
describe('GATE0 implementation contract (G1–G10)', () => {
  ...
  it('G0-02 — pool + queue mapper in doctor backend', () => {
    ...
    expect(fs.existsSync(path.join(root, 'issara-doctor/backend/appointmentQueueMapper.cjs'))).toBe(true);
  });

  it('G0-03 — realtime sync hooks for G6–G8', () => {
    expect(fs.existsSync(path.join(root, 'issara-doctor/frontend/services/useRealtimeSync.ts'))).toBe(true);
    const meeting = read('issara-doctor/frontend/pages/meetings/HealthMeeting.tsx');
