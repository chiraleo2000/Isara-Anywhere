...
describe('PHASE1 requirements contract', () => {
  ...
  it('PH1-02 — man-in-the-loop validation in EMR AI draft flow', () => {
    const draft = read('Isara-doctor-portal/frontend/utils/emrAiDraft.ts');
  ...
  it('PH1-04 — PostgreSQL-only appointment path (no GCS pool POST)', () => {
    const patientApi = read('Isara-patient-portal/backend/routes/appointments.ts');
  ...
  it('PH1-05 — Web Speech / post-meeting pipeline on meeting server', () => {
    expect(fs.existsSync(path.join(root, 'Izara-jitsi-server/backend/services/postMeetingPipeline.js'))).toBe(true);
