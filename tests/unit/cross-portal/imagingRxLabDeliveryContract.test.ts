...
describe('imagingRxLabDeliveryContract — source', () => {
  it('IRL-SRC — prescribing allergy + lab upload surfaces exist', () => {
    const prescribing = fs.readFileSync(
      path.join(root, 'issara-doctor/frontend/components/CompletePrescribing.tsx'),
      'utf8',
    );
    ...
    const lab = fs.readFileSync(
      path.join(root, 'issara-doctor/frontend/components/CompleteLabOrders.tsx'),
      'utf8',
    );
    expect(lab).toMatch(/lab-report-upload|\/api\/lab-orders/);

    const delivery = fs.readFileSync(
      path.join(root, 'issara-doctor/backend/services/documentDeliveryService.cjs'),
      'utf8',
    );
    ...
    const patientDelivery = fs.readFileSync(
      path.join(root, 'issara-patient/backend/services/documentDeliveryService.ts'),
      'utf8',
    );
