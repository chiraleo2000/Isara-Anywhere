 ...
 *
 * Patient download is under PHR: GET /api/phr/documents/:id/download
 * (not doctor-style /api/documents/:id/download). Soft-skip when path unavailable.
 */
...
async function login(base: string, email: string, password: string) {
  // Doctor npm-dev: /auth/login (auth :3011 via Vite). Patient: /api/auth/login.
  ...
  for (const loginPath of paths) {
    ...
    const token =
      ...
      ((body.data as { token?: string } | undefined)?.token);
    if (res.ok && token) return token;
    lastStatus = res.status;
    lastBody = body;
  }
  throw new Error(
    `login failed ${base} ${lastStatus} ${JSON.stringify(lastBody).slice(0, 200)}`,
  );
}
...
/** Patient portal serves download under /api/phr/...; keep doctor-style path as fallback. */
function patientDownloadCandidates(docId: string, downloadUrl?: string | null): string[] {
  const paths: string[] = [];
  if (downloadUrl) {
    paths.push(downloadUrl.startsWith('http') ? downloadUrl : `${PATIENT}${downloadUrl}`);
  }
  paths.push(
...
async function tryPatientDownload(
  patientToken: string,
  docId: string,
  downloadUrl?: string | null,
): Promise<Response | null> {
  for (const url of patientDownloadCandidates(docId, downloadUrl)) {
    ...
    async () => {
      ...
      for (const url of endpoints) {
        ...
        const body = (await res.json().catch(() => ({}))) as {
          document?: { id?: string; downloadUrl?: string; download_url?: string };
          id?: string;
          data?: { id?: string };
        };
        if (res.ok) {
          ...
          downloadUrl =
            body.document?.downloadUrl || body.document?.download_url || null;
          if (docId) break;
        }
      }
      ...
      if (!docId) {
        ...
        for (const url of [`${PATIENT}/api/phr/documents`, `${PATIENT}/api/documents`]) {
          ...
          const withId = Array.isArray(docs) ? docs.find((d) => d?.id) : undefined;
          if (withId?.id) {
            docId = withId.id;
            downloadUrl = withId.downloadUrl || withId.download_url || null;
            break;
          }
        }
      }
      ...
      if (!docId) {
        console.log('SKIP byteShareChain patient path: no document upload endpoint accepted payload');
        return;
      }
      const dl = await tryPatientDownload(patientToken, docId, downloadUrl);
      if (!dl) {
        console.log(
          'SKIP byteShareChain patient path: upload/download path unavailable (tried /api/phr/documents/:id/download)',
        );
        return;
      }
  ...
  it(
    ...
    async () => {
      ...
      for (const doc of docs.slice(0, 10)) {
        if (doc.id && (doc.download_url || doc.downloadUrl || doc.sourceType || doc.source_type)) {
          const dl = await tryPatientDownload(
            patientToken,
            doc.id,
            doc.downloadUrl || doc.download_url || null,
          );
          if (dl?.status === 200) {
