...
function readFrontendSources(): string {
  const dirs = [
    path.join(root, 'issara-doctor/frontend'),
    path.join(root, 'issara-patient/frontend'),
  ];
  const chunks: string[] = [];
...
describe('pageElementContract — P0 UI controls in code', () => {
  ...
  it('PEC-QUEUE — queue action buttons in AppointmentQueueCard', () => {
    const card = fs.readFileSync(
      path.join(root, 'issara-doctor/frontend/components/AppointmentQueueCard.tsx'),
      'utf8',
    );
  ...
  it('PEC-MSG — patient message composer send button', () => {
    const composer = fs.readFileSync(
      path.join(root, 'issara-doctor/frontend/components/PatientMessageComposer.tsx'),
      'utf8',
    );
  ...
  it('PEC-MEET-SHARE — guest URL hints + validate summary CTA', () => {
    const room = fs.readFileSync(
      path.join(root, 'issara-doctor/frontend/pages/meetings/MeetingRoom.tsx'),
      'utf8',
    );
    expect(room).toContain('data-testid="guest-join-url-hint"');
    expect(room).toContain('data-testid="guest-token-url-hint"');
    const results = fs.readFileSync(
      path.join(root, 'issara-doctor/frontend/pages/meetings/MeetingResults.tsx'),
      'utf8',
    );
  ...
  it('PEC-RECORD — patient record viewer tab testids', () => {
    const viewer = fs.readFileSync(
      path.join(root, 'issara-doctor/frontend/components/PatientRecordViewer.tsx'),
      'utf8',
    );
