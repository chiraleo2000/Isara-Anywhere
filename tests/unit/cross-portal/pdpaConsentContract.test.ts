...
describe('PDPA consent contract', () => {
  ...
  it('PDPA-02 — mainApiServer uses shared resolveMedicalRecordConsent', () => {
    ...
    expect(server).toMatch(/resolveMedicalRecordConsent\(PostgresDataService\.pool/);
    expect(server).toMatch(/resolveMedicalRecordConsent\(pool, patientId, doctorId\)/);
    expect(server).toMatch(/resolveDoctorId\(req\)/);
  });
});
