...
describe('Combined workflows actions (CWA)', () => {
  it('CWA-01 — patient book appointment API', () => {
    expect(read('issara-patient/backend/routes/appointments.ts')).toMatch(/router\.post/);
  });

  it('CWA-02 — doctor confirm appointment flow', () => {
    expect(read('issara-doctor/frontend/pages/meetings/HealthMeeting.tsx')).toMatch(/confirm/i);
  });

  it('CWA-03 — meeting join config cross-service', () => {
    expect(read('issara-doctor/frontend/utils/jitsiMeetingConfig.ts')).toMatch(/join-config/);
  });

  it('CWA-04 — post-meeting pipeline persists summary', () => {
    expect(read('issara-jitsi/backend/services/postMeetingPipeline.js')).toMatch(/summary|persist/i);
  });
  ...
  it('CWA-07 — patient dashboard join meeting CTA', () => {
    expect(read('issara-patient/frontend/pages/DashboardPage.tsx')).toMatch(/dashboard-join-meeting/);
  });

  it('CWA-08 — admin assign chain in health meeting', () => {
    expect(read('issara-doctor/frontend/pages/meetings/HealthMeeting.tsx')).toMatch(/assign|assignedDoctor/i);
  });

  it('CWA-09 — notification workflow cross-portal', () => {
    expect(read('issara-patient/backend/services/notificationService.ts')).toMatch(/notification/i);
  });
