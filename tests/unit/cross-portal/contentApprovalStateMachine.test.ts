...
describe('contentApprovalStateMachine — source', () => {
  it('CAS-SRC — medical content workflow mentions approved / draft status', () => {
    ...
    expect(svc).toMatch(/submitContentForReview|status/);
    expect(svc).toMatch(/approved|draft/);

    const types = fs.readFileSync(
      path.join(root, 'Isara-doctor-portal/frontend/types/contentTypes.ts'),
      'utf8',
    );
    expect(types).toMatch(/draft/);

    const visibility = fs.readFileSync(
      path.join(root, 'Isara-doctor-portal/backend/lib/contentVisibility.cjs'),
      'utf8',
    );
    expect(visibility).toMatch(/published|approved/);
  });
});
