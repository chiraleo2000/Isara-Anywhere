...
describe('clinical component structure guard (v1.7.48)', () => {
  it('CompleteEMREditor delegates chrome to EmrEditorChrome', () => {
    const emr = read('issara-doctor/frontend/components/CompleteEMREditor.tsx');
    expect(emr).toMatch(/import \{ EmrEditorChrome \} from '\.\/emr-editor\/EmrEditorChrome'/);
    expect(emr).toMatch(/<EmrEditorChrome/);
    expect(fs.existsSync(path.join(root, 'issara-doctor/frontend/components/emr-editor/EmrEditorChrome.tsx'))).toBe(true);
  });

  it('CompletePrescribing delegates chrome to PrescribingModalChrome', () => {
    const rx = read('issara-doctor/frontend/components/CompletePrescribing.tsx');
    ...
    expect(fs.existsSync(path.join(root, 'issara-doctor/frontend/components/prescribing/PrescribingModalChrome.tsx'))).toBe(true);
  });

  it('LiveTranscription delegates view to LiveTranscriptionView', () => {
    const lt = read('issara-doctor/frontend/components/LiveTranscription.tsx');
    ...
    expect(fs.existsSync(path.join(root, 'issara-doctor/frontend/components/transcription/LiveTranscriptionView.tsx'))).toBe(true);
    expect(fs.existsSync(path.join(root, 'issara-doctor/frontend/components/transcription/transcriptTypes.ts'))).toBe(true);
  });

  it('MeetingResults uses ValidationAction type alias (S4323)', () => {
    const mr = read('issara-doctor/frontend/pages/meetings/MeetingResults.tsx');
