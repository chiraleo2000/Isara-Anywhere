...
describe('Clinical document delivery contract', () => {
  ...
  it('DocumentDeliveryService exists on both portals', () => {
    expect(fs.existsSync(path.join(root, 'issara-doctor/backend/services/documentDeliveryService.cjs'))).toBe(true);
    expect(fs.existsSync(path.join(root, 'issara-patient/backend/services/documentDeliveryService.ts'))).toBe(true);
  });
  ...
  it('patient portal exposes documents API routes', () => {
    ...
    const phr = fs.readFileSync(path.join(root, 'issara-patient/backend/routes/phr.ts'), 'utf8');
    expect(index).toContain("app.use('/api/phr', phrRoutes)");
    expect(phr).toContain("router.get('/documents'");
    expect(phr).toContain("router.get('/documents/:id/download'");
    expect(index).toContain('/api/meetings/:id/recording-download');
  });

  it('doctor portal publishes EMR to health-logs with auth in CompleteEMREditor', () => {
    const emr = fs.readFileSync(path.join(root, 'issara-doctor/frontend/components/CompleteEMREditor.tsx'), 'utf8');
    expect(emr).toContain('Authorization');
    expect(emr).toMatch(/health-logs/);
  });

  it('PUT /api/emr/:id registered in mainApiServer', () => {
    const api = fs.readFileSync(path.join(root, 'issara-doctor/backend/mainApiServer.cjs'), 'utf8');
    expect(api).toContain("app.put('/api/emr/:emrId'");
    expect(api).toContain('documentDeliveryService.cjs');
    expect(api).toContain('patient_doctor_messages');
    expect(api).toContain('notifyDocumentDelivered');
    expect(api).toContain("DOCUMENT_DELIVERED_TYPE = 'document_delivered'");
  });
