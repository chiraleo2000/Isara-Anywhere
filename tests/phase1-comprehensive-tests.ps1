...
$TestUsers = @{
    ...
    Patient2 = @{ Email = "Somchai.Mankong@gmail.com"; Password = "P@ssw0rd"; Id = "PATIENT-SOMCHAI"; Name = "สมชาย มั่นคง" }
    Patient3 = @{ Email = "Anan.Khayanrian@gmail.com"; Password = "P@ssw0rd"; Id = "PATIENT-ANAN"; Name = "อนันต์ ขยันเรียน" }
    Doctor = @{ Email = "doctor.test@izara.com"; Password = "IzaraDoctor@2024"; Id = "DOC-TEST-001"; Name = "นพ. ทดสอบ" }
    Admin = @{ Email = "admin.test@izara.com"; Password = "IzaraAdmin@2024"; Id = "ADMIN-001"; Name = "นพ. ผู้ดูแล" }
}
...
$patientPages = @(
    @{ Name = "P1: หน้าหลัก (Dashboard)"; Url = "$($Urls.PatientPortal)/api/phr/PATIENT-DEMO"; Token = $patientToken },
    @{ Name = "P1: นัดหมาย (Appointments)"; Url = "$($Urls.PatientPortal)/api/appointments?patientId=PATIENT-DEMO"; Token = $patientToken },
    @{ Name = "P1: ปรึกษา AI (AI Status)"; Url = "$($Urls.PatientPortal)/api/ai/status"; Token = $patientToken },
    @{ Name = "P1: คลังความรู้สุขภาพ (Medical Content)"; Url = "$($Urls.PatientPortal)/api/content/medical"; Token = $patientToken },
    @{ Name = "P1: ประวัติสุขภาพ (PHR)"; Url = "$($Urls.PatientPortal)/api/phr/PATIENT-DEMO"; Token = $patientToken },
    @{ Name = "P1: เส้นทางสุขภาพ (Health Timeline)"; Url = "$($Urls.PatientPortal)/api/phr/PATIENT-DEMO/timeline"; Token = $patientToken },
    @{ Name = "P1: PDPA & Living Will"; Url = "$($Urls.PatientPortal)/api/pdpa/consents/PATIENT-DEMO"; Token = $patientToken },
    @{ Name = "P1: ตั้งค่า (Settings/Status)"; Url = "$($Urls.PatientPortal)/api/pdpa/status"; Token = $patientToken },
...
$patient2Pages = @(
    @{ Name = "P2: หน้าหลัก (Dashboard)"; Url = "$($Urls.PatientPortal)/api/phr/PATIENT-SOMCHAI"; Token = $patient2Token },
    @{ Name = "P2: นัดหมาย (Appointments)"; Url = "$($Urls.PatientPortal)/api/appointments?patientId=PATIENT-SOMCHAI"; Token = $patient2Token },
    @{ Name = "P2: ประวัติสุขภาพ (PHR)"; Url = "$($Urls.PatientPortal)/api/phr/PATIENT-SOMCHAI"; Token = $patient2Token },
...
$patient3Pages = @(
    @{ Name = "P3: หน้าหลัก (Dashboard)"; Url = "$($Urls.PatientPortal)/api/phr/PATIENT-ANAN"; Token = $patient3Token },
    @{ Name = "P3: นัดหมาย (Appointments)"; Url = "$($Urls.PatientPortal)/api/appointments?patientId=PATIENT-ANAN"; Token = $patient3Token },
    @{ Name = "P3: ประวัติสุขภาพ (PHR)"; Url = "$($Urls.PatientPortal)/api/phr/PATIENT-ANAN"; Token = $patient3Token },
...
$sharedPages = @(
    @{ Name = "แพทย์ (Doctors List)"; Url = "$($Urls.PatientPortal)/api/doctors"; Token = $patientToken }
)
...
$doctorPages = @(
    @{ Name = "แดชบอร์ด (Dashboard)"; Url = "$($Urls.DoctorPortal)/api/dashboard/DOC-TEST-001"; Token = $doctorToken },
    @{ Name = "ตารางนัดหมาย (Appointments)"; Url = "$($Urls.DoctorPortal)/api/appointments?doctorId=DOC-TEST-001"; Token = $doctorToken },
    @{ Name = "Doctors List (includes availability)"; Url = "$($Urls.DoctorPortal)/api/doctors"; Token = $doctorToken },
    @{ Name = "ผู้ป่วย (Patients)"; Url = "$($Urls.DoctorPortal)/api/patients"; Token = $doctorToken },
    @{ Name = "นัดหมาย & ประชุม (Appointments & Meetings)"; Url = "$($Urls.DoctorPortal)/api/appointments?doctorId=DOC-TEST-001&includeMeetings=true"; Token = $doctorToken },
    @{ Name = "ที่ปรึกษาแพทย์ (Consultants)"; Url = "$($Urls.DoctorPortal)/api/consultants"; Token = $doctorToken },
    @{ Name = "เนื้อหาทางการแพทย์ (Medical Content)"; Url = "$($Urls.DoctorPortal)/api/content/medical"; Token = $doctorToken },
    @{ Name = "ทรัพยากรทางคลินิก (Clinical Resources)"; Url = "$($Urls.DoctorPortal)/api/clinical-resources"; Token = $doctorToken },
    @{ Name = "คิวผู้ป่วย (Patient Queue)"; Url = "$($Urls.DoctorPortal)/api/queue/doctor/DOC-TEST-001"; Token = $doctorToken },
    ...
    @{ Name = "แพทย์ (Doctors List)"; Url = "$($Urls.DoctorPortal)/api/doctors"; Token = $doctorToken },
...
$adminPages = @(
    @{ Name = "แดชบอร์ด (Dashboard)"; Url = "$($Urls.DoctorPortal)/api/dashboard/ADMIN-001"; Token = $adminToken },
    @{ Name = "ตารางนัดหมาย (Appointments)"; Url = "$($Urls.DoctorPortal)/api/appointments?adminView=true"; Token = $adminToken },
    @{ Name = "Doctors List (includes availability)"; Url = "$($Urls.DoctorPortal)/api/doctors"; Token = $adminToken },
    @{ Name = "ผู้ป่วย (Patients)"; Url = "$($Urls.DoctorPortal)/api/patients"; Token = $adminToken },
    @{ Name = "นัดหมาย & ประชุม (Appointments & Meetings)"; Url = "$($Urls.DoctorPortal)/api/appointments?adminView=true&includeMeetings=true"; Token = $adminToken },
    @{ Name = "ที่ปรึกษาแพทย์ (Consultants)"; Url = "$($Urls.DoctorPortal)/api/consultants"; Token = $adminToken },
    @{ Name = "เนื้อหาทางการแพทย์ (Medical Content)"; Url = "$($Urls.DoctorPortal)/api/content/medical"; Token = $adminToken },
    @{ Name = "ทรัพยากรทางคลินิก (Clinical Resources)"; Url = "$($Urls.DoctorPortal)/api/clinical-resources"; Token = $adminToken },
    @{ Name = "จัดการแพทย์ (Manage Doctors)"; Url = "$($Urls.DoctorPortal)/api/doctors"; Token = $adminToken },
    @{ Name = "อนุมัติแพทย์ใหม่ (Pending Doctors)"; Url = "$($Urls.DoctorPortal)/api/admin/pending-doctors"; Token = $adminToken },
...
$summaryBody = @{
    appointmentId = $jitsiAppointmentId
    transcript = "Doctor: สวัสดีครับ คุณสมชาย วันนี้มาด้วยอาการอะไรครับ`nPatient: เหนื่อยง่าย ขาบวม มาประมาณ 1 สัปดาห์`nDoctor: น้ำหนักเพิ่มขึ้นมั้ยครับ`nPatient: เพิ่มขึ้น 2 กิโลครับ`nDoctor: BP 145/90, ขาบวม grade 2"
    language = "th"
}
...
$emrBody = @{
    ...
    soap = @{
        subjective = "ผู้ป่วยมาด้วยอาการเหนื่อยง่าย ขาบวม 1 สัปดาห์"
...
$instructionBody = @{
    ...
    appointmentId = "APT-TEST-001"
    instructions = @{
        medications = @("Metformin 500mg วันละ 2 ครั้ง หลังอาหาร", "Losartan 50mg วันละครั้ง")
        lifestyle = @("ควบคุมอาหารเค็ม", "ดื่มน้ำวันละ 1.5 ลิตร", "พักผ่อนให้เพียงพอ")
        followUp = "นัดพบแพทย์อีก 2 สัปดาห์"
        warnings = @("หากขาบวมมากขึ้นให้มาพบแพทย์ทันที")
    }
}
...
$e2eBookingBody = @{
    ...
    time = "14:00"
    type = "consultation"
    reason = "ติดตามผลเบาหวานและความดัน"
    symptoms = @{
        chief = "เหนื่อยง่าย ขาบวม"
        duration = "2 weeks"
    }
    letSystemDecideMeeting = $true
}
...
$transcriptE2E = @{
    appointmentId = $e2eAppointmentId
    transcript = @"
Doctor: สวัสดีครับคุณสมชาย วันนี้มาด้วยอาการอะไรครับ
Patient: เหนื่อยง่ายครับหมอ ขาบวม มาประมาณ 2 สัปดาห์แล้ว
Doctor: น้ำหนักเพิ่มขึ้นมั้ยครับ
Patient: เพิ่มขึ้นประมาณ 3 กิโลครับ
Doctor: ผมจะวัดความดันนะครับ... BP 148/92 สูงกว่าปกตินิดหน่อย
Doctor: ขาบวม grade 2 ทั้งสองข้าง
Doctor: น่าจะต้องปรับยาความดันและตรวจเลือดดูการทำงานของไตครับ
"@
    language = "th"
}
...
$instructE2E = @{
    ...
    instructions = @{
        medications = @(
            "Metformin 500mg - รับประทานวันละ 2 ครั้ง หลังอาหาร เช้า-เย็น",
            "Losartan 50mg - รับประทานวันละ 1 ครั้ง หลังอาหารเช้า (ยาใหม่)"
        )
        lifestyle = @(
            "ลดอาหารเค็ม เช่น น้ำปลา ซีอิ๊ว อาหารหมักดอง",
            "ดื่มน้ำวันละ 1.5-2 ลิตร",
            "ยกขาสูงเวลานอน"
        )
        followUp = "นัดพบแพทย์อีก 2 สัปดาห์ เพื่อดูผลเลือดและประเมินอาการ"
        warnings = @(
            "หากขาบวมมากขึ้นหรือหายใจเหนื่อยขึ้น ให้มาพบแพทย์ทันที",
            "หากมีอาการเวียนศีรษะหลังกินยา Losartan ให้แจ้งแพทย์"
        )
    }
}
...
$chatContextBody = @{
    doctorId = "DOC-TEST-001"
    message = "ผู้ป่วยเบาหวานร่วมกับ CKD stage 3b ควรปรับยา Metformin อย่างไร"
    patientContext = @{
        patientId = "PATIENT-SOMCHAI"
