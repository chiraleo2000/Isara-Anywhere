  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (res.ok()) {
        ...
        lastStatus = `http-not-ok`;
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      if (/Target page, context or browser has been closed/i.test(msg)) throw err;
      lastStatus = `error:${msg.slice(0, 80)}`;
    }
    await new Promise((r) => setTimeout(r, 2_000));
  }
  throw new Error(`waitMeetingEnded timeout after ${timeoutMs}ms (lastStatus=${lastStatus}, key=${meetingKey})`);
}
