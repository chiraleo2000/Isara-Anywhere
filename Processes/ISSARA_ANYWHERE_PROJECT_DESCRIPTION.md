# Issara Anywhere — Project Description (New Sibling Layout)
...
**Issara Anywhere** (Izara / อิสรา) is a **three-portal telemedicine platform** for Thai healthcare. It connects patients, doctors, and admins through video consultations with AI-assisted clinical workflows using the **Man-in-the-Loop** principle:
...
| Area | Summary |
| ---- | ------- |
| Patient portal | Book appointments, join video visits, manage PHR / PDPA / living will, AI health chat |
| Doctor portal | Schedule, EMR, prescribe, labs, queue, AI clinical studio, admin approval |
| Meeting server | Lobby, transcript, recording, Gemini post-meeting SOAP pipeline |
...
| Run all locally | `npm run dev` (concurrently jitsi + patient + doctor) |
...
| Layout | `MainLayout` — sidebar + header (notifications, theme, language, avatar) |
...
**Key routes (see ENRICH-9 pages for step-level detail):**
...
| Entry | `frontend/` |
| Roles | Doctor + Admin (RBAC on admin routes) |
...
**Modals (clinical workflow):** EMR Editor · Prescribing · Lab Orders · Patient Record Viewer · Gemini AI Studio
...
### Patient API (`issara-patient` — Express TypeScript `:3004`)
...
### Doctor Auth + Main API (`issara-doctor`)

| Process | Port | Entry |
| ------- | ---- | ----- |
| Auth server | **3011** | `backend/authServer.cjs` — login, register, session |
| Main API | **3009** | `backend/mainApiServer.cjs` — clinical + admin APIs |
...
| Responsibilities | Room create/join/end, Izara lobby, transcript segments, chat, guest invites, recordings, Gemini post-meeting pipeline |
...
| Port | Service | Repo |
| ---- | ------- | ---- |
| **3004** | Patient Express API | `issara-patient` |
| **3005** | Patient Vite SPA (+ proxies API) | `issara-patient` |
| **3009** | Doctor Main API | `issara-doctor` |
| **3010** | Doctor Vite SPA | `issara-doctor` |
| **3011** | Doctor Auth API | `issara-doctor` |
...
**Open:** Patient `http://localhost:3005` · Doctor `http://localhost:3010` · Meeting health `http://localhost:3020/health`
...
| Mode | When to use |
| ---- | ----------- |
| **npm run dev** | Day-to-day feature work, hot reload, Playwright headed UI |
...
| [Separated_Workflows_And_Functions.md](Separated_Workflows_And_Functions.md) | Process A–U by feature |
...
| [Clinical_Document_Delivery_Workflows.md](Clinical_Document_Delivery_Workflows.md) | Post-visit document delivery |
...
| Portal | Index |
| ------ | ----- |
| Patient | [Pages/Patient-Portal/00_Patient_Portal_Overview.md](Pages/Patient-Portal/00_Patient_Portal_Overview.md) |
| Doctor | [Pages/Doctor-Portal/00_Doctor_Portal_Overview.md](Pages/Doctor-Portal/00_Doctor_Portal_Overview.md) |
...
*Issara Anywhere project description for New-Isara-Anywhere sibling layout · July 2026*
