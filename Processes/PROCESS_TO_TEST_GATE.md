...
**Ops (2026-07-17 close-out baseline `20260717-1300`):** Local green retained from `20260716-1318`. Cloud re-proved on GCE VM Postgres `35.240.157.230` / `DB_SSL=false` — never Cloud SQL. Cloud Run uses discrete `DB_*` only (**do not mount** Secret Manager `database-url`). Tag **v1.7.62**. Evidence: [LOCAL_GREEN_CHECKPOINT.md](../reports/LOCAL_GREEN_CHECKPOINT.md), [CLOUD_GREEN_CHECKPOINT.md](../reports/CLOUD_GREEN_CHECKPOINT.md), [CLOUD_FULL_COVERAGE_RESULTS.md](../reports/CLOUD_FULL_COVERAGE_RESULTS.md).
...
**Fix All Gate Issues status (2026-07-17, baseline `20260717-1300`):** Phase 1–2 local green retained (`20260716-1318`). Phase 3 close-out: removed `DATABASE_URL` Cloud Run mount; prefer `DB_HOST` in app boot; synced Secret Manager `db-password` to GCE; `--clear-cloudsql-instances` on patient/doctor/meeting; deploy **v1.7.62** + traffic shift + GATE0 + showup — see [CLOUD_GREEN_CHECKPOINT.md](../reports/CLOUD_GREEN_CHECKPOINT.md).
...
5. `npm run cloud:deploy` (tag e.g. `v1.7.62`) + traffic shift + `cloud:smoke` + `verify:gate0` + `test:gate:ui-showup` + `test:cloud:full`
