...
> **เอกสารภาษาไทย** — สร้างอัตโนมัติจาก `Medicine_Content_Processes.md`  
> **ต้นฉบับภาษาอังกฤษ:** [`Medicine_Content_Processes.md`](../Medicine_Content_Processes.md)  
...
11. [พอร์ทัลผู้ป่วย Access](#11-ผู้ป่วย-portal-access)
...
- **Location**:
  - พอร์ทัลแพทย์: `MedicalContent.tsx` page
  - พอร์ทัลผู้ป่วย: `MedicalContentLibrary.tsx` → "คลังความรู้สุขภาพ" tab in Health Studio
...
- **Audience**: Doctors only (with ผู้ดูแลระบบ approval workflow)


- **Location**: พอร์ทัลแพทย์: `ClinicalResources.tsx` page
...
| --------- | ------------------------------------------ | ----------------------------------- |
| ผู้ป่วย | ✅ Read published only | ❌ No access |
| แพทย์ | ✅ Create, Edit, Delete own | ✅ Create, Edit, Submit for approval |
| | ✅ Publish (direct or via approval) | ⏳ รอดำเนินการ approval required |
| | ✅ Read all published | ✅ Read approved only |
| ผู้ดูแลระบบ | ✅ Full CRUD | ✅ Approve/Reject |
| | ✅ Approve/Reject submissions | ✅ View all (including รอดำเนินการ) |
...
## 4. ขั้นตอนการทำงาน Steps


### Medical Content ขั้นตอนการทำงาน (Patient-Facing)
...
1. **Publish** → Content visible in พอร์ทัลผู้ป่วย คลังความรู้สุขภาพ
...
### Clinical Resources ขั้นตอนการทำงาน (Doctor-Facing)
...
1. **ผู้ดูแลระบบ Review** (MANDATORY)
...
| การกระทำ | พอร์ทัลแพทย์ | GCS | พอร์ทัลผู้ป่วย |
| -------- | --------------- | ----- | ---------------- |
| Create Draft | ✅ Write | ✅ Store | ❌ Not visible |
| Submit for Review | ✅ Update สถานะ | ✅ Store | ❌ Not visible |
| Approve | ✅ ผู้ดูแลระบบ การกระทำ | ✅ Store | ❌ Not visible |
| Publish | ✅ สถานะ change | ✅ Store | ✅ Visible |
| Update Published | ✅ Edit | ✅ Store | ✅ Updated |
| Archive | ✅ สถานะ change | ✅ Store | ❌ Hidden |
...
3. **Audit Logging**: Log ALL สถานะ changes for compliance
4. **เวอร์ชัน Control**: Always save previous เวอร์ชัน to history before updates
...
1. **ผู้ดูแลระบบ Detection** (line ~93)

    ```tsx
    const isAdmin = user?.email?.includes('admin') || user?.role === 'admin';
    ```

1. **รอดำเนินการ Approvals Button** (Header - visible to ผู้ดูแลระบบ only when pendingCount > 0)
   - Shows notification badge with count
   - Opens รอดำเนินการ list modal on click

1. **Submit for Approval Button** (View Modal)
   - Visible for draft/rejected articles
   - Calls `handleSubmitForApproval()` to change สถานะ to 'รอดำเนินการ'

1. **Review Button** (View Modal)
   - Visible to ผู้ดูแลระบบ only for รอดำเนินการ articles
   - Opens approval modal

1. **Approval Modal** (ผู้ดูแลระบบ only)
   - Preview of article content
   - Approval comment field
   - Reject with reason field
   - Approve/Reject การกระทำ buttons

1. **รอดำเนินการ List Modal** (ผู้ดูแลระบบ only)
   - Lists all รอดำเนินการ articles
...
- [ ] แพทย์ can create draft content


- [ ] แพทย์ can submit content for approval


- [ ] ผู้ดูแลระบบ sees รอดำเนินการ approvals


- [ ] ผู้ดูแลระบบ can approve/reject with feedback


- [ ] Published content appears in พอร์ทัลผู้ป่วย
...
- [ ] เวอร์ชัน history preserved on updates
...
2. **Real-time การแจ้งเตือน**
   - Push การแจ้งเตือน for approval สถานะ
...
| สถานะ | คำอธิบาย | Who Can See | Next Actions |
| -------- | ------------- | ------------- | -------------- |
...
## 14. PostgreSQL ฐานข้อมูล Architecture


### ฐานข้อมูล Tables for Medicine Content

| Table | Purpose | Key Columns |
| ----- | ------- | ----------- |
| **medical_content** | ผู้ป่วย-facing health articles | id, title_thai, title_english, content_thai, content_english, category, tags (JSONB), author_id, author_name, สถานะ (draft/รอดำเนินการ/published/rejected/archived), image_url, view_count |
| **clinical_resources** | แพทย์-facing guidelines | id, title_thai, title_english, content_thai, content_english, category, specialty, guideline_year, tags (JSONB), สถานะ (รอดำเนินการ/approved), author_id, approved_by, approved_at |
...
| **audit_logs** | All content CRUD operations | id, user_id, การกระทำ, entity_type, entity_id, details (JSONB) |
...
| ----------- | ------- | -------------- | -------- |
| Local Docker | พอร์ทัลแพทย์ (3010) | Full CRUD + reviews | izara-postgres:5432 |
| Local Docker | พอร์ทัลผู้ป่วย (3005) | Read published only | izara-postgres:5432 |
...
| - | -------- | ----- | --------- |
| 1 | Create health article | แพทย์ | medical_content, audit_logs |
| 2 | Submit for review | แพทย์ | medical_content |
| 3 | Approve/publish | ผู้ดูแลระบบ | medical_content, knowledge_base, audit_logs |
| 4 | Reject article | ผู้ดูแลระบบ | medical_content, audit_logs |
| 5 | ผู้ป่วย browses articles | ผู้ป่วย | medical_content (read) |
| 6 | Create clinical resource | แพทย์ | clinical_resources, audit_logs |
| 7 | AI queries drug interactions | Meeting Server | drugs, cds_logs |
| 8 | AI RAG search | Doctor/Meeting | knowledge_base |
| 9 | ICD-10 code lookup | แพทย์ | icd10_codes (read) |
| 10 | Archive published content | ผู้ดูแลระบบ | medical_content, audit_logs |