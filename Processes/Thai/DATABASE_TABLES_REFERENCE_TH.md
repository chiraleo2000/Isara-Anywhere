...
> **เอกสารภาษาไทย** — สร้างอัตโนมัติจาก `DATABASE_TABLES_REFERENCE.md`  
> **ต้นฉบับภาษาอังกฤษ:** [`DATABASE_TABLES_REFERENCE.md`](../DATABASE_TABLES_REFERENCE.md)  
...
3. [ผู้ป่วย Clinical Data](#3-ผู้ป่วย-clinical-data)
4. [แพทย์ Management](#4-แพทย์-management)
5. [นัดหมาย & Meetings](#5-นัดหมาย--meetings)
6. [Clinical Records](#6-clinical-records)
7. [Content & Reference Data](#7-content--reference-data)
8. [AI, Knowledge & การแจ้งเตือน](#8-ai-knowledge--การแจ้งเตือน)
9. [Mobile, Sync & User Preferences](#9-mobile-sync--user-preferences)
10. [Extended Clinical Modules](#10-extended-clinical-modules)
11. [Audit & ผู้ดูแลระบบ](#11-audit--ผู้ดูแลระบบ)
...
All three services (พอร์ทัลผู้ป่วย, พอร์ทัลแพทย์, Meeting Server) share one PostgreSQL database. Tables are grouped below by domain. **Workflow** links point to the primary process document that reads or writes the table.
...
**นัดหมาย สถานะ flow:** `pending` → `in_pool` → `ai_matched` → `doctor_claimed` → `confirmed` → `in_progress` → `completed` (or `cancelled` / `declined`)

**Meeting สถานะ flow:** `scheduled` → `waiting` → `active` / `in_progress` → `completed` → results pipeline → `ready_for_patient`
...
## 13. Table → ขั้นตอนการทำงาน Index
