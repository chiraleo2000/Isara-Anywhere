...
> **เอกสารภาษาไทย** — สร้างอัตโนมัติจาก `Clinical_Resources_&_Medical_Library_Workflows.md`  
> **ต้นฉบับภาษาอังกฤษ:** [`Clinical_Resources_&_Medical_Library_Workflows.md`](../Clinical_Resources_&_Medical_Library_Workflows.md)  
> **อัปเดต:** 9 กรกฎาคม 2569 · รัน `python scripts/sync-processes-thai.py` เพื่อสร้างใหม่

**เวอร์ชัน:** 1.6.0
**อัปเดตล่าสุด:** March 31, 2026
**สถานะ:** ✅ PostgreSQL Implementation
...
The Clinical Resources page provides access to medical guidelines, research papers, and evidence-based study materials. Doctors can create content that requires ผู้ดูแลระบบ approval before publishing.
...
- **APPROVE/REJECT**: Review รอดำเนินการ content submissions

- **READ**: View all resources regardless of สถานะ

- **COMMENT**: Provide feedback on submissions

- **VIEW รอดำเนินการ**: See รอดำเนินการ approval queue with count badge
...
- **SUBMIT**: Submit drafts for ผู้ดูแลระบบ approval
...
### ฐานข้อมูล Tables for Clinical Resources

| Table | Purpose | Key Columns |
| ----- | ------- | ----------- |
| **clinical_resources** | Main content storage | id, title_thai, title_english, content_thai, content_english, category, specialty, guideline_year, source, tags (JSONB), สถานะ (รอดำเนินการ/approved), image_url, author_id, author_name, approved_by, approved_at |
...
| **ai_chat_history** | แพทย์ AI queries about clinical resources | user_id, session_id, role, content, context (JSONB), embedding (vector) |
| **audit_logs** | All create/update/approve/reject actions | user_id, การกระทำ, entity_type='clinical_resource', entity_id, details (JSONB) |
...
| ----------- | ------- | --- | -------- |
| Local Docker | พอร์ทัลแพทย์ | localhost:3010 | izara-postgres:5432 |
| Local Docker | พอร์ทัลผู้ป่วย | localhost:3005 | izara-postgres:5432 |
| Production | พอร์ทัลแพทย์ | Cloud Run (asia-southeast1) | 35.240.157.230:5432 |
| Production | พอร์ทัลผู้ป่วย | Cloud Run (asia-southeast1) | 35.240.157.230:5432 |
...
| # | Scenario | ผู้ดำเนินการ | สถานะ Flow | DB Tables |
| - | -------- | ----- | ----------- | --------- |
| 1 | Create draft | แพทย์ | → draft | clinical_resources, audit_logs |
| 2 | Submit for approval | แพทย์ | draft → รอดำเนินการ | clinical_resources, audit_logs |
| 3 | ผู้ดูแลระบบ approves | ผู้ดูแลระบบ | รอดำเนินการ → approved | clinical_resources, knowledge_base, audit_logs |
| 4 | ผู้ดูแลระบบ rejects | ผู้ดูแลระบบ | รอดำเนินการ → rejected | clinical_resources, audit_logs |
| 5 | Edit published | แพทย์ | approved → รอดำเนินการ | clinical_resources, audit_logs |
| 6 | AI chat queries resource | แพทย์ | — (read) | knowledge_base, ai_chat_history |
| 7 | ผู้ป่วย views resource | ผู้ป่วย | — (read) | clinical_resources |
| 8 | CDS references guideline | Meeting Server | — (read) | knowledge_base |
| 9 | Delete resource | แพทย์ (owner) | any → deleted | clinical_resources, knowledge_base, audit_logs |
...
| Content not รอดำเนินการ | "Resource is not รอดำเนินการ approval" | Check สถานะ |


## Correlation with Admin Users

1. **Approval Workflow**: Admins see รอดำเนินการ badge, review content
2. **Feedback System**: ผู้ดูแลระบบ comments visible to content creator
3. **Audit Trail**: All changes tracked with user info
4. **สถานะ Control**: Only ผู้ดูแลระบบ approval changes รอดำเนินการ → published
...
3. **เวอร์ชัน Notes**: Add meaningful change notes when editing
