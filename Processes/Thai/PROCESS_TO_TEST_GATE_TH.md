...
**Latest:** 2026-06-30 — Round 6 local: self-hosted Jitsi (`meet.localhost:8443`), แพทย์ moderator JWT, manual recording; phases 0–5 + phase 3 ledger round 2 P0=0; LAN gate **BLOCKED** (Ubuntu `192.168.10.239` unreachable). See `reports/signoff/round6-local-lan-signoff-2026-06-30.md`.
...
4. `gcloud builds submit` with `_MIN_INSTANCES=0,_MAX_INSTANCES=2` ต่อด้วย `npm run test:cloud:deploy-gate` — **BLOCKED** รอดำเนินการ user approval
