...
> **เอกสารภาษาไทย** — สร้างอัตโนมัติจาก `BACKEND_AND_FRONTEND_OVERVIEW.md`  
> **ต้นฉบับภาษาอังกฤษ:** [`BACKEND_AND_FRONTEND_OVERVIEW.md`](../BACKEND_AND_FRONTEND_OVERVIEW.md)  
...
4. [Data Flow — ผู้ป่วย ↔ แพทย์ ↔ Meeting](#4-data-flow--ผู้ป่วย--แพทย์--meeting)
...
### 2.1 ผู้ป่วย API (`issara-patient`)
...
| Channel / pattern | Use |
| ----------------- | --- |
| Socket.IO on ผู้ป่วย API | การแจ้งเตือน, นัดหมาย updates |
| PG NOTIFY → broadcast | Cross-service sync (`appointment:updated`, `notification:new`, etc.) |
| Meeting Socket.IO (`:3020`) | Lobby wait, transcript view-only, meeting สถานะ (via meeting client) |
...
| Scheduling | `appointments`, นัดหมาย pool related rows |
| Content / AI | content tables, AI conversation history (as implemented) |
| Clinical read | `prescriptions`, lab result tables, ลงนามแล้ว EMR → PHR delivery |
| Comms | `notifications`, `device_tokens` |

---


### 2.2 แพทย์ Auth + Main API (`issara-doctor`)
...
| Area | Purpose |
| ---- | ------- |
| Login / logout | แพทย์ & ผู้ดูแลระบบ JWT sessions |
| Registration | แพทย์ signup → approval workflow |
...
| Channel / pattern | Use |
| ----------------- | --- |
| Socket.IO (แพทย์ backend) | Queue, schedule, notification events |
| Meeting Socket.IO (`:3020`) | Host-present, lobby admit/reject, transcript, summary-ready |
| PG NOTIFY | Same shared channels as ผู้ป่วย for cross-portal consistency |
...
| Domain | Tables |
| ------ | ------ |
| Users / RBAC | `users`, `sessions`, แพทย์ profile tables |
...
| ผู้ดูแลระบบ | approval fields on `users`, ผู้ดูแลระบบ นัดหมาย views |
...
| หลังประชุม AI | Summary / SOAP / CDS / ผู้ป่วย instructions pipeline |
| `/health` | Health check |
...
| Lobby admit / reject | แพทย์ ↔ Server | Izara lobby control |
...
| AI outputs | summaries linked to นัดหมาย / EMR draft fields |
...
ENRICH-9 ขั้นตอน docs: [Pages/Patient-Portal/00_Patient_Portal_Overview.md](Pages/Patient-Portal/00_Patient_Portal_Overview.md)
...
| คำสั่งตรวจแล็บ | Lab & imaging |
| ผู้ป่วย Record Viewer | PHR / EMR / EHR |
| Gemini AI Studio | Clinical chat + calculators |

ENRICH-9 ขั้นตอน docs: [Pages/Doctor-Portal/00_Doctor_Portal_Overview.md](Pages/Doctor-Portal/00_Doctor_Portal_Overview.md)
...
| ------- | ------ | -------------- |
| Meeting room (host/patient) | แพทย์ + ผู้ป่วย | [01_Meeting_Room.md](Pages/Meeting-Server/01_Meeting_Room.md) |
| Meeting results / AI summary | แพทย์ | [02_Meeting_Results.md](Pages/Meeting-Server/02_Meeting_Results.md) |
| EMR from นัดหมาย | แพทย์ | [03_Emr_Appointment_Page.md](Pages/Meeting-Server/03_Emr_Appointment_Page.md) |
...
## 5. Shared ฐานข้อมูล Touchpoints
...
| ---------------- | --------------------- | -------------------- |
| ผู้ป่วย API | users (ผู้ป่วย), phr, นัดหมาย (request), consents, หนังสือแสดงเจตจำนอง | doctors, content, ลงนามแล้ว EMR/Rx/labs, การแจ้งเตือน |
| แพทย์ Auth + Main | users (doctor/admin), EMR, Rx, labs, นัดหมาย (confirm/claim), queue | phr (consented), meeting summaries, knowledge_base |
...
| [Separated_Workflows_And_Functions.md](Separated_Workflows_And_Functions.md) | ฟีเจอร์ processes A–U |
...
| [Pages/Patient-Portal/00_Patient_Portal_Overview.md](Pages/Patient-Portal/00_Patient_Portal_Overview.md) | ผู้ป่วย ENRICH-9 hub |
| [Pages/Doctor-Portal/00_Doctor_Portal_Overview.md](Pages/Doctor-Portal/00_Doctor_Portal_Overview.md) | แพทย์ ENRICH-9 hub |
