...
> **เอกสารภาษาไทย** — สร้างอัตโนมัติจาก `TWO_ROUND_CLOUD_TESTING.md`  
> **ต้นฉบับภาษาอังกฤษ:** [`TWO_ROUND_CLOUD_TESTING.md`](../TWO_ROUND_CLOUD_TESTING.md)  
> **อัปเดต:** 9 กรกฎาคม 2569 · รัน `python scripts/sync-processes-thai.py` เพื่อสร้างใหม่


**Release tag:** `v1.7.54`  
**อัปเดตล่าสุด:** June 30, 2026 (Round 6 — self-hosted Jitsi local, cost-opt cloudbuild, LAN blocked)
...
| Self-hosted Jitsi local | **PASS** | `meet.localhost:8443`; แพทย์ JWT moderator; Q+R phase:4 |
...
| Cloud deploy gate E2E (A+D+Q) | **PASS** exit **0** | **21/21** headed — D15 ผู้ดูแลระบบ pool auth fix (per-role `refresh-e2e-auth`, `ensureDoctorPortalAuthenticated`, API retry); Q01 host-present + participants defer |
...
**W9 D15 fix (test helpers):** `refreshAuthStorageStateForRole` avoids cross-portal session invalidation on cloud; D3 reinjects ผู้ดูแลระบบ+แพทย์ before pool; D15 uses `waitForContent(..., 'admin')`, `ensureDoctorPortalAuthenticated`, `pageRequestGetWithAuthRetry` / `pageRequestPatchWithAuthRetry` against `DOCTOR_URL`.

**Cloud Run revisions (dev-testing) — Round 5:**

| Service | Revision |
|---------|----------|
| พอร์ทัลแพทย์ | `izara-doctor-portal-dev-testing-00156-87g` |
| พอร์ทัลผู้ป่วย | `izara-patient-portal-dev-testing-00124-rr8` |
...
| Cloud doc screenshots | **รอดำเนินการ** | Blocked until cloud deploy + A09 fix on Cloud Run |
...
| Service | Revision |
|---------|----------|
| พอร์ทัลแพทย์ | `izara-doctor-portal-dev-testing-00155-6k5` |
| พอร์ทัลผู้ป่วย | `izara-patient-portal-dev-testing-00123-zlm` |
...
| Service | Revision |
|---------|----------|
| พอร์ทัลแพทย์ | `izara-doctor-portal-dev-testing-00154-sw9` |
| พอร์ทัลผู้ป่วย | `izara-patient-portal-dev-testing-00122-nwr` |
...
Round 2b: capture PNGs from Cloud Run only (`BASELINE_VISUAL=1`, groups P/W/D/E/F/S), ต่อด้วย rebuild guides.
...
3. Meeting API checks:
   ...
   - `GET /api/meetings/{appointmentId}/join-config?role=doctor` (with แพทย์ JWT)
...
| Test | คำอธิบาย |
|------|-------------|
| Q01 | แพทย์ HOST (internal JWT) + ผู้ป่วย + named guest → lobby admit → 10s stable → end |
...
| ขั้นตอน | ผู้ดำเนินการ | Expect |
|------|--------|--------|
| E10c | แพทย์ | `jitsi-meeting-container` + `iframe` visible in MeetingRoom |
...
| Group | Content |
|-------|---------|
| D | นัดหมาย pool, มอบหมาย, queue counts |
| E | Meeting create, แพทย์ room, ผู้ป่วย room, lobby admit, guest join |
| F | PHR / lab after E2a |
...
- ผู้ป่วย sees “waiting for moderator”: redeploy **Meeting Server + both portals** (v1.7.12+).
- Guest no video: แพทย์ must be **host-present** first; guest flow waits `host-ready` ต่อด้วย mounts Jitsi.
...
- **แพทย์:** HOST — joins first, `host-present`, Izara lobby admit-all.
...
| ขั้นตอน | Requirement |
|------|-------------|
| Q01e | แพทย์ clicks `admit-all-btn` (UI), not API-only |
