...
> **เอกสารภาษาไทย** — สร้างอัตโนมัติจาก `PostgreSQL_Database_Architecture.md`  
> **ต้นฉบับภาษาอังกฤษ:** [`PostgreSQL_Database_Architecture.md`](../PostgreSQL_Database_Architecture.md)  
...
**Schema:** `izara_phase1`
**สถานะ:** ✅ Production schema — 53+ tables (see [DATABASE_TABLES_REFERENCE.md](DATABASE_TABLES_REFERENCE.md))
...
| นัดหมาย & Meetings | นัดหมาย, meeting_records, meeting_transcripts, meeting_chats, meeting_invites, recording_share_tokens, appointment_ai_suggestions | 7 |
| Clinical Data | emr, emr_records, ใบสั่งยา, lab_orders, imaging_orders, patient_instructions, health_timeline | 7 |
| Content & Knowledge | medical_content, clinical_resources, icd10_codes, drugs, knowledge_base, ai_document_analysis | 6 |
| AI & Decision Support | ai_chat_history, ai_chat_memory, transcript_embeddings, cds_logs, ai_validations, การแจ้งเตือน | 6 |
...
| Audit & ผู้ดูแลระบบ | audit_logs, access_audit, admin_actions | 3 |
...
| patient_id | VARCHAR(50) FK→users | ผู้ป่วย |
...
| is_shared_with_doctors | BOOLEAN | แพทย์ visibility |
| เวอร์ชัน | INTEGER | เวอร์ชัน number |
| audit_log | JSONB DEFAULT '[]' | Access trail |


#### `living_will_versions` — เวอร์ชัน history

| Column | Type | คำอธิบาย |
| ------ | ---- | ----------- |
| id | VARCHAR(50) PK | เวอร์ชัน ID |
| patient_id | VARCHAR(50) FK→users | ผู้ป่วย |
| เวอร์ชัน | INTEGER NOT NULL | เวอร์ชัน number |
| data | JSONB NOT NULL | Full snapshot |
| note | TEXT | เวอร์ชัน note |
...
| doctor_id | VARCHAR(50) | Specific แพทย์ |
| data_types | JSONB DEFAULT '["all"]' | Data scopes |
| สถานะ | VARCHAR(20) | รอดำเนินการ/active/revoked |
...
#### `doctor_profiles` — Extended แพทย์ information
...
#### `doctors` — ผู้ป่วย-facing แพทย์ listing

| Column | Type | คำอธิบาย |
| ------ | ---- | ----------- |
| id | VARCHAR(50) PK | แพทย์ listing ID |
...
| doctor_id | VARCHAR(50) FK→users | แพทย์ |
...
#### `doctor_reviews` — ผู้ป่วย ratings
...
| doctor_id | VARCHAR(50) FK→users | แพทย์ |
| patient_id | VARCHAR(50) FK→users | ผู้ป่วย |
| appointment_id | VARCHAR(50) | Related นัดหมาย |
...
| Column | Type | คำอธิบาย |
| ------ | ---- | ----------- |
| id | VARCHAR(50) PK | ใบสั่งยา ID |
| emr_id | VARCHAR(50) FK→emr | Linked EMR |
| appointment_id | VARCHAR(50) FK→นัดหมาย | นัดหมาย |
| patient_id | VARCHAR(50) FK→users | ผู้ป่วย |
| doctor_id | VARCHAR(50) FK→users | Prescribing แพทย์ |
...
| สถานะ | VARCHAR(20) | draft/ลงนามแล้ว/dispensed |
...
| appointment_id | VARCHAR(50) FK→นัดหมาย | นัดหมาย |
| patient_id | VARCHAR(50) FK→users | ผู้ป่วย |
| doctor_id | VARCHAR(50) FK→users | Ordering แพทย์ |
...
| สถานะ | VARCHAR(20) | ordered/กำลังดำเนินการ/completed |
...
#### `medical_content` — ผู้ป่วย education articles
...
| author_id | VARCHAR(50) FK→users | Author (แพทย์) |
| สถานะ | VARCHAR(20) | draft/รอดำเนินการ/published |
| view_count | INTEGER DEFAULT 0 | Read count |


#### `clinical_resources` — แพทย์ reference materials
...
| สถานะ | VARCHAR(20) | รอดำเนินการ/approved |
| author_id | VARCHAR(50) FK→users | Author |
| approved_by | VARCHAR(50) FK→users | Approver (ผู้ดูแลระบบ) |
...
| description_english | TEXT | English คำอธิบาย |
| description_thai | TEXT | Thai คำอธิบาย |
...
| patient_id | VARCHAR(50) FK→users | ผู้ป่วย |
| doctor_id | VARCHAR(50) FK→users | Requesting แพทย์ |
...
| patient_id | VARCHAR(50) | ผู้ป่วย |
| doctor_id | VARCHAR(50) | แพทย์ |
| appointment_id | VARCHAR(50) | นัดหมาย |
| recommendation_type | VARCHAR(50) | Alert type |
| severity | VARCHAR(20) | info/warning/critical |
| title | VARCHAR(255) | Alert title |
| คำอธิบาย | TEXT | Alert details |
| guideline_source | VARCHAR(255) | Evidence source |
| doctor_decision | VARCHAR(20) | accepted/rejected/modified |


#### `ai_validations` — Man-in-the-Loop (แพทย์ตรวจสอบก่อนส่งถึงผู้ป่วย) log
...
| patient_id | VARCHAR(50) | Affected ผู้ป่วย |
| การกระทำ | VARCHAR(50) | การกระทำ performed |
| entity_type | VARCHAR(50) | Table/entity affected |
| entity_id | VARCHAR(50) | Entity ID |
| details | JSONB | การกระทำ details |
| old_value | JSONB | Previous state |
| new_value | JSONB | New state |
| ip_address | VARCHAR(45) | Client IP |
| user_agent | TEXT | Browser info |
| performed_by | VARCHAR(50) | ผู้ดำเนินการ reference |
...
| ------------ | ----- | ------ | --------------- |
| notify_appointment_change | นัดหมาย | INSERT, UPDATE, DELETE | `appointment:updated` |
| notify_emr_change | emr | INSERT, UPDATE | `emr:updated` |
| notify_prescription_change | ใบสั่งยา | INSERT, UPDATE | `prescription:updated` |
...
| ใบสั่งยา | View | Create/Sign | Create/Sign |
| คำสั่งตรวจแล็บ | View results | Create/View | Create/View |
