...
> **เอกสารภาษาไทย** — สร้างอัตโนมัติจาก `SECURITY_SCANNING.md`  
> **ต้นฉบับภาษาอังกฤษ:** [`SECURITY_SCANNING.md`](../SECURITY_SCANNING.md)  
...
**Pre-deploy gate** (`npm run test:local:pre-deploy-gate`) runs `sonar:lint` ต่อด้วย `security:scan` before portal lint and E2E.
...
|-----|-----------|----------|
| พอร์ทัลแพทย์ | `Isara-doctor-portal/` | `package-lock.json` |
| พอร์ทัลผู้ป่วย | `Isara-patient-portal/` | `package-lock.json` |
...
Last scan: **FAIL** on แพทย์-portal (HIGH+ policy).

| App | HIGH | MEDIUM | Suggested fix |
|-----|------|--------|---------------|
| แพทย์-portal | 2 | 0 | `npm install multer@2.1.1 react-router-dom@7.15.0` (in `Isara-doctor-portal/`) |
| ผู้ป่วย-portal | 0 | 1 | `npm install react-router-dom@6.30.4` (medium only — does not block) |
| jitsi-server | 0 | 1 | `npm install @google-cloud/speech@7.0.0` (medium only) |

**Phase 1 (P0 — unblock gates):**

- [ ] Upgrade `multer` 1.x → 2.1.1 in พอร์ทัลแพทย์ (API/upload regression test)
- [ ] Upgrade `react-router-dom` to ≥7.15.0 in พอร์ทัลแพทย์ (transitive `react-router` CVE-2026-42211)
- [ ] Re-run `npm run security:scan` and `npm run test:local:pre-deploy-gate`

**Phase 2 (P1 — hygiene):**

- [ ] พอร์ทัลผู้ป่วย `react-router-dom@6.30.4` (medium)
...
- [ ] Document multer 2.x migration in พอร์ทัลแพทย์ upload routes if API changed
...
- `PROCESS_TO_TEST_GATE.md` — gate ขั้นตอน mapping
