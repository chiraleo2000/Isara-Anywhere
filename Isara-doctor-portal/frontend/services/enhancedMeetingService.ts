...
if (genAI) {
  ...
  console.log(`   Meeting Server: ${meetingServerBase()}`);
} else {
...
export class EnhancedMeetingService {
  ...
  private async syncTranscriptToMeetingServer(
    ...
    try {
      const response = await fetch(`${meetingServerBase()}/api/meetings/transcript`, {
        method: 'POST',
  ...
  async requestMeetingServerSummary(meetingId: string): Promise<any> {
    try {
      const response = await fetch(`${meetingServerBase()}/api/meetings/${meetingId}/summary`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
