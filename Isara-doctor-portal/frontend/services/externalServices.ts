...
import { resolveJitsiDomain } from '../utils/jitsiMeetingConfig';
...
export class GoogleMeetService {
  ...
  private initializeGoogleAPIs() {
    ...
    if ((globalThis as any).gapiLoaded && (globalThis as any).gisLoaded) {
    ...
    // Load GAPI
    this.appendScriptOnce(
      'https://apis.google.com/js/api.js',
      () => {
        (globalThis as any).gapiLoaded = true;
        this.gapiLoad();
      },
      () => this.handleInitError('GAPI'),
    );
    ...
    this.appendScriptOnce(
      ...
      () => {
        (globalThis as any).gisLoaded = true;
        this.gisLoad();
      },
      () => this.handleInitError('GIS'),
    );
  }

  private appendScriptOnce(src: string, onload: () => void, onerror: () => void) {
