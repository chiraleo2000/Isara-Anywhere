...
// Back-compat for modules that read ALLOWED_ORIGINS
...
// A02 - CORS with strict origin checking (CORS_ORIGINS only)
app.use(cors({
...
async function pgCreateSession(userId, email, role, ip, userAgent, deviceId, options = {}) {
  if (!pgPool) return null;
  const invalidatePrevious = options.invalidatePreviousSessions !== false;
  try {
    const token = crypto.randomBytes(32).toString('hex');
    const expiresAt = new Date(Date.now() + 3 * 60 * 60 * 1000); // 3 hours (matches JWT_EXPIRES_IN)
    
    // Login: single active session. Token refresh: keep existing sessions (meetings in progress).
      ...
      try {
        sessionResult = await Promise.race([
          pgCreateSession(
            ...
            deviceId,
            {
              invalidatePreviousSessions: process.env.E2E_ALLOW_PARALLEL_SESSIONS !== '1',
            },
          ),
          new Promise((_, reject) =>
...
app.post('/auth/refresh',
  ...
  try {
    ...
    // Create new session without invalidating in-flight meeting sessions
    const sessionResult = await pgCreateSession(
