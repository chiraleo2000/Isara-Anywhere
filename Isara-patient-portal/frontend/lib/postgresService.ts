...
// Helper for meeting-server API (canonical Izara-jitsi-server)
async function meetingApiCall<T>(
  endpoint: string,
  options: RequestInit = {}
  ...
  try {
    ...
    const response = await fetch(`${resolveMeetingServerUrl()}${endpoint}`, {
      ...options,
      headers,
      credentials: 'include',
    });
    ...
    if (!response.ok) {
      return { success: false, error: data.error || data.message || 'Meeting API error' };
    }
...
export async function joinVideoAppointment(
  ...
  return meetingApiCall(`/api/meetings/${appointmentId}/lobby/join`, {
    method: 'POST',
    body: JSON.stringify({ participantId, participantName, role }),
  });
}

// ===== Video Meeting Functions (meeting-server canonical) =====
...
export async function createVideoMeeting(data: {
  ...
  return meetingApiCall('/api/meetings/create', {
    method: 'POST',
...
export async function getVideoMeeting(appointmentId: string): Promise<ApiResponse<{
  ...
  return meetingApiCall(`/api/meetings/${appointmentId}`);
}
