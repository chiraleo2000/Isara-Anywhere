      ...
      {showAddAllergy && (
        <div className={`p-4 rounded-xl mb-4 ${isDark ? 'bg-red-900/30 border border-red-700' : 'bg-red-50'}`}>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setShowAddAllergy(false); setNewAllergy(''); }} aria-label="ยกเลิก" title="ยกเลิก" className={`px-4 py-2 rounded-lg ${isDark ? 'bg-gray-700 text-gray-200 hover:bg-gray-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        {editingProfile ? (
          <div className="space-y-3">
            <div className="flex flex-wrap gap-2">
              {profileData.chronicConditions.map((condition) => (
                <span key={condition} className="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-sm flex items-center gap-1">
                  {condition}
                  <button onClick={() => removeChronicCondition(condition)} aria-label={`ลบ ${condition}`} title={`ลบ ${condition}`} className="hover:text-red-600">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
            ...
            <div className="flex gap-2">
              ...
              />
              <button onClick={addChronicCondition} aria-label="เพิ่มโรคประจำตัว" title="เพิ่มโรคประจำตัว" className="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-600">
