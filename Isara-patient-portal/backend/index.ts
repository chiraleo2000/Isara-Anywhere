...
import videoMeetingRoutes from './routes/video-meeting-proxy';
...
const ALLOWED_ORIGINS: string[] = [...izaraCorsPolicy.literals];
const CLOUD_RUN_PATTERN = izaraCorsPolicy.cloudRunPattern;
...
// A02 - CORS with origin validation (CORS_ORIGINS only)
app.use(cors({
  origin: corsOriginValidator,
  credentials: true,
...
// Runtime env-config.js — no-cache (Docker envsubst overwrites dist/env-config.js at startup)
if (process.env.NODE_ENV === 'production') {
  const envConfigCandidates = [
    path.join(__dirname, '../dist/env-config.js'),
    path.join(__dirname, '../../dist/env-config.js'),
    '/app/dist/env-config.js',
  ];
  app.get('/env-config.js', (_req: Request, res: Response) => {
    const envConfigPath = envConfigCandidates.find((p) => fs.existsSync(p));
    if (!envConfigPath) {
      return res.status(404).type('application/javascript').send('window.ENV = window.ENV || {};');
    }
    res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0');
    res.type('application/javascript');
    return res.sendFile(envConfigPath);
  });
}
...
try {
  ...
  httpServer.listen(PORT, () => {
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log(`🚀 Izara Patient Portal API Server v1.7.3`);
